...
- **Flow**: authenticated → straight into the lobby (`ensureJoined`). Guest → public existence-only card, **Log in** or **Create your player account** (nickname/email/password → registration endpoint). After registration the page auto-signs-in: the browser re-submits the just-typed credentials to `/login.do`, then navigates to the protected `ft_join_go` page (whose nav_to capture also serves the manual login path — `sysparm_goto_url` is ignored by this instance). A rejected auto-login falls back to the standard login on the protected page.
...
- **QR codes render locally** (`src/widgets/ft-game/lib.js` — vendored
  `qrcode@1.5.4`, MIT, bundled from npm source; drawn to a canvas on-device).
  The invite token never leaves the player's phone. `deploy-widget.mjs`
  prepends any widget's `lib.js` to its client script. If canvas rendering
  ever fails, the copy-link button and 4-char code still work.
