...
3. Registration endpoint (public Scripted REST): validates token ↔ game in `lobby` state → creates `sys_user` (email as user_name), grants exactly the `x_tekvo_famtriv.player` role, creates the Profile with the chosen nickname, and writes a `registration_log` row (user, game, ip) — the audit index for self-created accounts. On success the join page auto-signs-in: the visitor's own browser re-submits the just-typed credentials to the instance's standard login endpoint (same origin, one POST, both credential copies cleared immediately after dispatch), then navigates to the protected join page (`ft_join_go`) — landing them in the game. A rejected login degrades to the standard instance login on that protected page. Note: `/login.do?sysparm_goto_url` is ignored by this instance; all post-login routing relies on nav_to capture via the protected page.
